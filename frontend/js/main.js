...
(function() {
    ...
    async function handleTraceClick(e, clickedElement) {
        ...
        if (routingSession) {
            // Check if clicked on a same-net pad - trigger auto-route
            if (match && match.type === 'pad') {
                const padNet = parseInt(match.element.dataset.net, 10);
                if (padNet > 0 && padNet === routingSession.startNet) {
                    // Same net pad - auto-route to this destination
                    const target = getTargetCoordinates(e, match.element);
                    await autoRouteToPad(match.element, target.x, target.y);
                    return;
                }
            }
