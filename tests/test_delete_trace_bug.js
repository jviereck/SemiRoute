...
async function runTest() {
    console.log('Starting delete trace bug regression test...');
    ...
    try {
        ...
        const traceExistsAfterCreation = await page.evaluate(() => document.querySelector('.user-trace') !== null);
        if (!traceExistsAfterCreation) {
        ...
        if (!isSelected) {
            await page.screenshot({ path: `${SCREENSHOT_DIR}/delete_bug_not_selected.png` });
        ...
        if (traceDeleted) {
            ...
            await page.screenshot({ path: `${SCREENSHOT_DIR}/delete_bug_fail.png` });
        ...
        await page.screenshot({ path: `${SCREENSHOT_DIR}/delete_bug_error.png` });
        process.exit(1);
    } finally {
        await browser.close();
    }
